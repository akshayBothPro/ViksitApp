...
public class CalenderAdapter extends RecyclerView.Adapter<TimeLineViewHolder>  {

    private List<DailyTaskPOJO> calendarDataList;
    ...
    public CalenderAdapter(List<DailyTaskPOJO> calendarDataList,Context context) {
    ...
    public void onBindViewHolder(TimeLineViewHolder holder, int position) {
        ...
        DailyTaskPOJO calendarData = calendarDataList.get(position);

        holder.mDate.setText(sdf.format(calendarData.getStartDate()));
        holder.mMessage.setText(calendarData.getName());
        if(historyDate.contains(sdf.format(calendarData.getStartDate()))) {
            ...
            if(isToday(calendarData.getStartDate().getTimestamp())){
            ...
            try {
                if(ft.parse(ft.format(new Date())).compareTo(ft.parse(ft.format(calendarData.getStartDate()))) == 0){
            ...
            holder.mTimelineView.setMarker(ContextCompat.getDrawable(mContext, R.drawable.ic_marker), ContextCompat.getColor(mContext, R.color.gray_pill));
            holder.date.setText(datesdf.format(calendarData.getStartDate()));
            holder.month.setText(monthsdf.format(calendarData.getStartDate()));
            if(isToday(calendarData.getStartDate().getTimestamp())){
                holder.main_layout.setBackgroundColor(mContext.getResources().getColor(R.color.white));
            ...
            historyDate.add(sdf.format(calendarData.getStartDate()));
        }
        holder.date.setText(datesdf.format(calendarData.getStartDate()));
        holder.month.setText(monthsdf.format(calendarData.getStartDate()));
